(*  Title:      Pure/ex/Guess.thy
    Author:     Makarius
    Modified by:     Simon Wimmer

This is a modified version of the theory from Isabelle-2021-1-RC3.
The difference is that the 'qed' will print out the obtained facts as a completion hint.
...
theory Guess_Explore
  imports
    Explorer \<comment> \<open>We merely want to get the same 'quoting' behaviours (quotes vs cartouches).\<close>
...
text \<open>
  ...
  Some examples are in \<open>Pure/ex/Guess_Examples.thy\<close>.
\<close>
